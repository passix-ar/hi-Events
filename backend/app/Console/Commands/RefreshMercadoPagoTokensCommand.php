...
class RefreshMercadoPagoTokensCommand extends Command
{
    ...
    public function handle(
        ...
        /** @var AccountMercadopagoPlatformDomainObject $row */
        foreach ($expiring as $row) {
            if (! $this->refreshPlatform($row->getId(), $row->getAccountId())) {
                $failures++;
            }
        }
    ...
    private function refreshPlatform(int $id, int $accountId): bool
    {
        try {
            $platform = $this->platformRepository->findFirstWhere([
                AccountMercadopagoPlatformDomainObjectAbstract::ID => $id,
            ]);
